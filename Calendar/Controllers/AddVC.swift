...
class AddVC: UIViewController {
    ...
    @objc func saveButtonTapped() {
        ...
        let saveData = Expense(cost: self.expenseTextField.text ?? "",
                               ...
                               id: key)
        // 값이 같은지 다른지 확인한다
        if let expenses = self.expenses, expenses != saveData {
            // 만약 값이 다르다면 이 함수를 통해 서버의 값과 바꿔준다
            saveExpenseToServer(saveData)
        } else {
            // 만약 똑같다면 그냥 창을 닫는다
            self.dismiss(animated: true, completion: nil)
        }
        
         func saveExpenseToServer(_ expense: Expense) {
            // 만약 저장된 값과 다르다면 값을 바꿔준다
            do {
                let encoder = JSONEncoder()
                let jsonData = try encoder.encode(expense)
                guard let jsonObject = try JSONSerialization.jsonObject(with: jsonData, options: []) as? [String: Any] else {
                    print("Failed to convert 'Expense' object to dictionary.")
                    return
                }
                
//                let autoId = MemoFetcher.shared.ref.childByAutoId()
//                let key = autoId.key ?? ""
                MemoFetcher.shared.ref.child("\(expenses?.id ?? "")").setValue(jsonObject) { error, ref in
                    if let error = error {
                        print("Failed to save data to the server: \(error.localizedDescription)")
                    } else {
                ...
                print("Error while encoding 'Expense' object: \(error.localizedDescription)")
            }
        }
    }
}
